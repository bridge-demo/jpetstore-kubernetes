...
class Deploy:
    ...
    def __init__(self, deployId=uuid.uuid4().__str__(), technical_service_name="byo_ado_deploy", duration=100572, name="petstore_deployment", applicaitonUrl="http://jpetstore-web.cd6578cfa15a4488b1b8.eastus.aksapp.io/shop/index.do", provider="Azure", status="fail", environment="production", isProduction=True,deployUrl="http://13.82.103.214:8080/view/RedThread/job/redthread-petstore-deployment-template/71/console", release=f'release-{time.strftime("%Y.%m.%d")}'):
        ...
        self.providerhref = "https://dev.azure.com/jamesxavier2/ModernOpsDemoEng/_build?definitionId=8"
    ...
    def deploy_petstore_helmchart(dockerRepo, imageTag, mysqlUrl, mysqlUser, mysqlPassword, petstoreHost,  jenkinsHome="..", namespace="jppetstore" ):
        ...
        kubeConfigExists = file_exists("./pipeline-python/tmp_kube_config.yaml")
        ...
        cleanPetstore = f"kubectl delete job jpetstoredb --ignore-not-found -n {namespace} --kubeconfig ./pipeline-python/tmp_kube_config.yaml".split(" ")
        ...
        installNgnix = f"helm upgrade --install ingress-nginx ingress-nginx/ingress-nginx --namespace ingress-nginx --create-namespace --set controller.service.type=LoadBalancer --wait --kubeconfig ./pipeline-python/tmp_kube_config.yaml".split(" ")
        ...
        helmUpgradeCommand = f"helm upgrade --install --wait --set image.repository={dockerRepo} --set image.tag={imageTag} --set mysql.url={base64.b64encode(mysqlUrl.encode('utf-8')).decode()} --set mysql.username={base64.b64encode(mysqlUser.encode('utf-8')).decode()} --set mysql.password={base64.b64encode(mysqlPassword.encode('utf-8')).decode()} --set isDBAAS=True --set isLB=False --set httpHost={petstoreHost} --set ingress.hosts[0]={petstoreHost} --namespace={namespace} --create-namespace {namespace} --kubeconfig ./pipeline-python/tmp_kube_config.yaml {jenkinsHome}/modernpets/modernpets-0.1.5.tgz".split(" ")
    ...
    def post_data_into_tenant( self, deployToken: str, tenantUrl:str,  ):
        ...
        payload = {
            ...
            'provider': "Azure",
            'providerhref': "https://dev.azure.com/jamesxavier2/ModernOpsDemoEng/_build?definitionId=8", 
