...
def create_kubernetes_namespace(kubeconfigPath="tmp_kube_config", namespace="monitoring"):
    ...
    try:
        ...
        returncode = os.system(f"kubectl get ns {namespace} --kubeconfig ./kubeconfig.yaml ")
        
        namespaceAlreadyExists = returncode == 0
        if namespaceAlreadyExists:
            return

        returncode = os.system(f"kubectl create ns {namespace} --kubeconfig ./kubeconfig.yaml ")
        if returncode != 0:
